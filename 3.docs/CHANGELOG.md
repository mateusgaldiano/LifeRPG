...
- **Feat: o avatar agora muda pela CLASSE dominante do radar, não só pelo rank.** O atributo líder define a pasta (`wisdom-male`, `physical-female`…), cruzando com o gênero: 16 combinações × 6 ranks. Empate técnico entre atributos vira `desperto`; quem ainda não saiu do lugar é `novato`.
  ...
  - **Fallback em cadeia:** pasta da classe → pasta base do gênero **no mesmo rank** → rank E da base. Hoje 11 das 16 pastas de classe estão sem arte; sem a cadeia, o avatar simplesmente sumiria para quem caísse numa delas (o `onerror` antigo só tentava o rank E da *mesma* pasta, que também estaria vazia). Cada pasta que receber arte passa a valer sozinha, sem tocar no código.
- **Fix: rank A aparecia como rank E na tela social.** O `avatarFileMap` do `social.js` não tinha a chave `a`, então caía no default (`1.rank-e`) — busca, lista de amigos e ranking mostravam a arte errada para todo jogador rank A.
- **Fix: Nacional/Monarca apareciam como rank E no zoom do avatar.** O `prefixMap` do `openAvatarZoom` só conhecia `e..s`; qualquer rank acima montava um nome de arquivo inexistente (`1.rank-monarca.webp`) e caía no fallback do rank E.
- **Fix: o zoom chamava um Monarca de "RECRUTA".** O `titleMap` do `openAvatarZoom` não tinha os ranks acima de S, e o `defaultTitles` do `openPlayerProfile` não tinha o rank A (virava "Candidato").
  - Os três bugs acima eram **quatro cópias divergentes** do mesmo mapa rank→arquivo/título, cada uma com um buraco diferente. Agora há um mapa só em `game-math.js` (`AVATAR_RANK_FILE`, `RANK_TITLES`), coberto por testes.
- **Testes:** 16 novos casos em `tests/game-math.test.mjs` (27 no total) — classe dominante, limiar do novato, empate/epsilon, mapa de rank e a cadeia de fallback, incluindo regressão explícita dos bugs de rank A e Nacional/Monarca.
- **Nota:** na tela social o avatar segue usando a pasta base (sem classe) para todos. A `public_profiles` ainda não expõe `class_key`/`gender` dos outros jogadores, e aqueles `<img>` são criados sem cadeia de `onerror`. O avatar por classe para amigos e ranking depende dessas duas colunas.
...
- **Fix: usuária feminina via o avatar masculino.** A lógica do avatar (`updateAvatarImage`) estava correta (`gender === 'female'` → pasta feminina), mas o campo `gameState.gender` **nunca era sincronizado com a nuvem** — só existia no localStorage. Ao logar em outro aparelho, limpar o cache ou após um sync "nuvem vence", o gênero caía no default `'male'` e a usuária passava a ver o avatar do homem. Além disso, **não havia como trocar o gênero depois do onboarding**.
  ...
  - **Seletor no app:** nova seção **🧍 PERSONAGEM (AVATAR)** nas Configurações com ♂ Homem / ♀ Mulher / ⚧ Neutro. Ao escolher, atualiza o avatar na hora, salva local e empurra pra nuvem. É por aqui que a usuária corrige o avatar dela agora.

## [v2.5.24] — 2026-07-12
- **Fix (exploit sério): marcar/desmarcar a mesma tarefa dava lucro de XP/Ouro (dava pra subir de nível).** A conclusão somava o valor **com todos os multiplicadores** (tomo ×N, streak, sinergia, perk, grupo, rank), mas desmarcar removia só o valor **base** da quest — cada ciclo marcar→desmarcar deixava um saldo positivo. Reportado por usuário ("ganho 30, desmarco e perco 20; subi de nível assim"). Correção: a conclusão agora guarda na quest o valor exato concedido (`_xpAwarded`/`_goldAwarded`) e o desmarcar remove **exatamente isso** (recalcular seria frágil — os multiplicadores mudam com o tempo). `deductRewards` também passou a **reverter o level-up** (empresta do nível anterior) pra desmarcar devolver ao estado idêntico de antes. Corrigido nos dois caminhos: `toggleQuest` (checkbox) e `adjustWater` (quests de contador, ex.: água). Quests concluídas antes do patch têm fallback pro valor base ao desmarcar.
- **Fix (exploit vizinho): recompensas de "dia completo" davam pra farmar por toggle.** `checkAllDailies` incrementava streak (+ contador de escudo + perks `mente_diamante`/`o_sistema`) **toda vez** que as dailies ficavam completas — então marcar/desmarcar a última daily inflava o streak indefinidamente (e o streak alimenta o multiplicador de XP, compondo o bug acima). Agora essas recompensas só valem **uma vez por dia** (trava por data em `_dailiesRewardedDate`); o incremento diário legítimo do streak continua normal na virada do dia.
...
- **Balance: Tributo agora escala com o nível da skill.** Antes era +5 fixo, que vira pó no fim de jogo (a curva de skill XP é exponencial) — exatamente onde o Tributo deveria ser um dreno relevante. Agora concede `clamp(15% do xpToNext da skill, 5, 30)` XP. Continua um péssimo negócio vs. hábitos grátis (é um ralo de ouro), mas empurra skills atrasadas de verdade.
